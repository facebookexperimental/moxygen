 ...
 public:
  ...
  void onSubscribe(
    ...
    clientSession->subscribeOk(
        {subReq.subscribeID, std::chrono::milliseconds(0)});
  ...
  bool catchup(
    ...
    time_t t = start.group * 60 + std::max(start.object, 1ul) - 1;
    ...
    if (end <= now) {
      clientSession->subscribeFin({subscribeID, start.group, start.object});
      return true;
    }
    return false;
  }
