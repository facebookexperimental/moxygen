...
class DatePublisher : public Publisher {
 public:
  ...
  // Runs on publisherEvb_. Publishes one second of date data to all subscribers.
          ...
          [&publisher, cancelSrc, &evb]() -> folly::coro::Task<void> {
            ...
            while (!cancelSrc.isCancellationRequested()) {
              ...
              co_await folly::coro::co_withCancellation(
                  cancelSrc.getToken(), folly::coro::sleep(std::chrono::seconds(1), &timekeeper));
            }
          }))
      .start();

  moxygen::SignalHandler handler(
      &evb, [&cancelSrc, &server, &relayClient](int) {
        cancelSrc.requestCancellation();
        if (relayClient) {
          relayClient->getEventBase()->add(
