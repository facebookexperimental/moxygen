        ...
        if (sub.second.fullTrackName == fullVideoTrackName_ && videoPub_) {
          if (item->isEOF &&
            ...
            moqClient_.getEventBase()->runInEventBaseThread(
                [this, subscribeID(sub.second.subscribeID)] {
                  latestVideo_.object++,
                      XLOG(DBG1) << "Closing group because EOF. objId="
                                 << latestVideo_.object;
                  if (videoSgPub_) {
          ...
          if (item->type == FlvSequentialReader::MediaType::VIDEO) {
            // Video
            moqClient_.getEventBase()->runInEventBaseThread(
                [this, item, subscribeID(sub.second.subscribeID)] {
                  publishVideoItem(subscribeID, std::move(item));
                });
          }
        }
