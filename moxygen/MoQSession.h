...
class MoQSession : public MoQCodec::Callback {
 public:
  ...
  class ControlVisitor : public boost::static_visitor<> {
   public:
    ...
    virtual void operator()(AnnounceError annErr) const {
      XLOG(INFO) << "AnnounceError ns=" << annErr.trackNamespace
                 << " code=" << annErr.errorCode
                 << " reason=" << annErr.reasonPhrase;
    }
    ...
    virtual void operator()(SubscribeFin subscribeFin) const {
      XLOG(INFO) << "SubscribeFin subID=" << subscribeFin.subscribeID;
    }

    virtual void operator()(SubscribeRst subscribeRst) const {
      XLOG(INFO) << "SubscribeRst subID=" << subscribeRst.subscribeID;
    }
  ...
  void announceError(AnnounceError annErr);
  ...
  void subscribeFin(SubscribeFin subFin);
  void subscribeRst(SubscribeRst subRst);
 ...
 private:
  ...
  void onSubscribeFin(SubscribeFin subscribeFin) override;
  void onSubscribeRst(SubscribeRst subscribeRst) override;
