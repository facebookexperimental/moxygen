    ...
    switch (parseState_) {
      ...
      case ParseState::FRAME_PAYLOAD: {
        if (curFrameType_ == FrameType::OBJECT_STREAM ||
            curFrameType_ == FrameType::OBJECT_PREFER_DATAGRAM) {
...
folly::Expected<folly::Unit, ErrorCode> MoQCodec::parseFrame(
  ...
  switch (curFrameType_) {
    ...
    case FrameType::SUBSCRIBE_FIN: {
      auto res = parseSubscribeFin(cursor);
      if (res) {
        if (callback_) {
          callback_->onSubscribeFin(std::move(res.value()));
        }
      } else {
        return folly::makeUnexpected(res.error());
      }
    ...
    case FrameType::SUBSCRIBE_RST: {
      auto res = parseSubscribeRst(cursor);
      if (res) {
        if (callback_) {
          callback_->onSubscribeRst(std::move(res.value()));
        }
      } else {
        return folly::makeUnexpected(res.error());
      }
