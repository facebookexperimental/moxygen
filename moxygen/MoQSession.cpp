...
MoQSession::SubscribeCallback MoQSession::getSubscribeCallback(
    TrackAlias trackAlias) {
  ...
  if (trackIt == subTracks_.end()) {
    ...
    return SubscribeCallback(nullptr);
  }
  return SubscribeCallback(trackIt->second);
}

MoQSession::FetchCallback MoQSession::getFetchCallback(
    SubscribeID subscribeID) {
  ...
  if (trackIt == fetches_.end()) {
    ...
    return FetchCallback(nullptr);
  }
  return FetchCallback(trackIt->second);
}

std::shared_ptr<TrackConsumer> MoQSession::SubscribeCallback::get() const {
  return trackReceiveState_ ? trackReceiveState_->callback_ : nullptr;
}

void MoQSession::SubscribeCallback::reset() {
  if (trackReceiveState_) {
    trackReceiveState_->callback_.reset();
  }
}

folly::CancellationToken MoQSession::CallbackBase::getCancelToken() const {
  return (trackReceiveState_) ? trackReceiveState_->getCancelToken()
                              : folly::CancellationToken();
}

std::shared_ptr<FetchConsumer> MoQSession::FetchCallback::get() const {
  return trackReceiveState_ ? trackReceiveState_->fetchCallback_ : nullptr;
}

void MoQSession::FetchCallback::reset() {
  if (trackReceiveState_) {
    trackReceiveState_->fetchCallback_.reset();
  }
}
...
class SubgroupCodecCallback : public MoQObjectStreamCodec::ObjectCallback {
 public:
  ...
  void setCallback(MoQSession::SubscribeCallback cb) {
  ...
  void onObjectStatus(
    ...
    switch (status) {
      ...
      case ObjectStatus::OBJECT_NOT_EXIST:
        ...
        break;
      case ObjectStatus::GROUP_NOT_EXIST:
        res = callback->groupNotExists(group, subgroup, true);
        subgroupCallback_.reset();
        break;
      case ObjectStatus::END_OF_GROUP:
 ...
 private:
  ...
  MoQSession::SubscribeCallback callback_;
...
class FetchCodecCallback : public MoQObjectStreamCodec::ObjectCallback {
 public:
  ...
  void setCallback(MoQSession::FetchCallback cb) {
    callback_ = std::move(cb);
  }
  ...
  void onObjectPayload(Payload payload, bool objectComplete) override {
    ...
    auto res = callback->objectPayload(
        std::move(payload),
        /*finSubgroup=*/false);
    if (!res) {
      error_ = std::move(res.error());
  ...
  void onObjectStatus(
    ...
    switch (status) {
      case ObjectStatus::NORMAL:
        break;
      case ObjectStatus::OBJECT_NOT_EXIST:
        res = callback->objectNotExists(group, subgroup, objectID);
        break;
      case ObjectStatus::GROUP_NOT_EXIST:
        res = callback->groupNotExists(group, subgroup, false);
        break;
      case ObjectStatus::END_OF_GROUP:
        res = callback->endOfGroup(group, subgroup, objectID, false);
        break;
      case ObjectStatus::END_OF_TRACK_AND_GROUP:
        res = callback->endOfTrackAndGroup(group, subgroup, objectID);
        callback_.reset();
        break;
      case ObjectStatus::END_OF_SUBGROUP:
        break;
 ...
 private:
  std::shared_ptr<MoQSession> session_;
  MoQSession::FetchCallback callback_;
  folly::Optional<MoQPublishError> error_;
};
...
class DispatchCallback : public MoQObjectStreamCodec::ObjectCallback {
 public:
  ...
  void onSubgroup(
      ...
      uint64_t subgroup,
      Priority priority) override {
    auto callback = session_->getSubscribeCallback(alias);
    if (!callback) {
      error_ = MoQPublishError(
          MoQPublishError::CANCELLED, "Subgroup for unknown track");
      return;
    }
    token_ = folly::CancellationToken::merge(token_, callback.getCancelToken());
    codec_.setCallback(&subgroupCallback_);
    subgroupCallback_.setCallback(std::move(callback));
    subgroupCallback_.onSubgroup(alias, group, subgroup, priority);
  }

  void onFetchHeader(SubscribeID subscribeID) override {
    auto callback = session_->getFetchCallback(subscribeID);
    ...
    fetchCallback_.setCallback(std::move(callback));
...
void MoQSession::onSubscribeDone(SubscribeDone subscribeDone) {
  ...
  auto trackReceiveStateIt = subTracks_.find(trackAliasIt->second);
  if (trackReceiveStateIt != subTracks_.end()) {
    auto trackReceiveState = trackReceiveStateIt->second;
    subTracks_.erase(trackReceiveStateIt);
    trackReceiveState->subscribeDone(std::move(subscribeDone));
  } else {
...
void MoQSession::onFetchOk(FetchOk fetchOk) {
  ...
  auto trackReceiveState = fetchIt->second;
  trackReceiveState->fetchOK();
  if (trackReceiveState->allDataReceived()) {
...
folly::coro::Task<MoQSession::SubscribeResult> MoQSession::subscribe(
  ...
  auto trackReceiveState = std::make_shared<TrackReceiveState>(
      fullTrackName, subID, callback, nullptr);
...
folly::coro::Task<folly::Expected<SubscribeID, FetchError>> MoQSession::fetch(
  ...
  controlWriteEvent_.signal();
  auto trackReceiveState = std::make_shared<TrackReceiveState>(
      fullTrackName, subID, nullptr, fetchCallback);
  ...
  trackReceiveState->setNewObjectTimeout(std::chrono::seconds(2));
  auto res = co_await trackReceiveState->fetchReady();
  XLOG(DBG1) << __func__
...
void MoQSession::onDatagram(std::unique_ptr<folly::IOBuf> datagram) {
  ...
  XCHECK(alias);
  auto callback = getSubscribeCallback(*alias).get();
  if (callback) {
