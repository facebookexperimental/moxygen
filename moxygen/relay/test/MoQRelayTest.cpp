...
TEST_F(MoQRelayTest, DuplicateSubgroupReplacesActiveConsumers) {
  ...
  // Duplicate beginSubgroup - should reset v1 consumers and return new forwarder
  ...
  // Close the new subgroup cleanly before teardown to avoid reset during cleanup
...
TEST_F(MoQRelayTest, DuplicateSubgroupSkipsTombstonedSubscriber) {
  ...
  // On duplicate: sub A's v1 consumer gets reset; sub B is tombstoned (no reset)
  ...
  // Duplicate beginSubgroup: sub A gets reset+new, sub B is skipped (tombstoned)
  auto sgForwarder2 = publishConsumer->beginSubgroup(0, 0, 0);
  EXPECT_TRUE(sgForwarder2.hasValue());
  EXPECT_NE(sgForwarder1.value(), sgForwarder2.value());
