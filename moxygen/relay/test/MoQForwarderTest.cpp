...
protected:
  ...
  std::shared_ptr<MoQForwarder::Subscriber> addSubscriber(
    ...
    sub.locType = locType;
    return forwarder.addSubscriber(std::move(session), sub, std::move(consumer));
  }
};
...
public:
  ...
  void onEmpty(MoQForwarder*) override { forwarderRef_.reset(); }
...
TEST_F(MoQForwarderTest, ForwarderOnlyCreatesSubgroupsBeforeObjectData) {
  ...
  auto sub1Handle = addSubscriber(*forwarder, subscriber1, mockConsumer1, RequestID(1));
  ...
  auto sub2Handle = addSubscriber(*forwarder, subscriber2, mockConsumer2, RequestID(2));
  ASSERT_NE(sub2Handle, nullptr);

  EXPECT_TRUE(subgroup->beginObject(1, 9, 0).hasValue());

  auto sub3Handle = addSubscriber(*forwarder, subscriber3, mockConsumer3, RequestID(3));
...
TEST_F(MoQForwarderTest, RemoveSessionResetsOpenSubgroups) {
  ...
  auto subHandle = addSubscriber(*forwarder, subscriber, consumer, RequestID(1));
...
TEST_F(MoQForwarderTest, SubscriberUnsubscribeDoesNotReceiveNewObjects) {
  ...
  EXPECT_TRUE(
      forwarder->publishDone({RequestID(1), PublishDoneStatusCode::TRACK_ENDED, 0, "track ended"})
          .hasValue()
  );
...
TEST_F(MoQForwarderTest, DataOperationCancelledWhenAllSubscribersFail) {
  ...
  auto forwarder = std::make_shared<MoQForwarder>(kFwdTestTrackName);
  for (size_t i = 0; i < 2; ++i) {
    auto h = addSubscriber(*forwarder, subscribers[i], consumers[i], RequestID(i + 1));
    ASSERT_NE(h, nullptr);
  }
...
TEST_F(MoQForwarderTest, LateJoinerGetsSubgroupAfterTombstone) {
  ...
  auto sub1Handle = addSubscriber(*forwarder, subscriber1, consumer1, RequestID(1));
  ASSERT_NE(sub1Handle, nullptr);
  ...
  auto sub2Handle = addSubscriber(*forwarder, subscriber2, consumer2, RequestID(2));
  ASSERT_NE(sub2Handle, nullptr);
...
TEST_F(MoQForwarderTest, EndOfSubgroupHardErrorDoesNotCrash) {
  ...
  auto subHandle = addSubscriber(*forwarder, subscriberSession, consumer, RequestID(1));
  ASSERT_NE(subHandle, nullptr);
