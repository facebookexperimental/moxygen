...
folly::Expected<folly::Unit, MoQPublishError> MoQCache::CacheGroup::cacheObject(
  ...
  if (it != objects.end()) {
    ...
    if (status == ObjectStatus::NORMAL && it->second->complete &&
        ((!payload && it->second->payload) ||
         (payload && !it->second->payload) ||
         (payload && it->second->payload &&
          payload->computeChainDataLength() !=
              it->second->payload->computeChainDataLength()))) {
    ...
    it->second->extensions = extensions;
...
class MoQCache::SubscribeWriteback : public TrackConsumer {
 public:
  ...
  folly::Expected<folly::Unit, MoQPublishError> objectStream(
    ...
    auto cacheRes = track_.getGroup(header.group)
                        .cacheObject(
                            header.subgroup,
                            header.id,
                            header.status,
