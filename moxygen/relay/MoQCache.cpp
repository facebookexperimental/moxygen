...
bool MoQCache::evictForByteLimitIfNeeded() {
  ...
  while (totalCachedBytes_ > targetBytes && !trackLRU_.empty()) {
    ...
    if (!track.groupLRU.empty()) {
      ...
      // Track has groups but none are evictable (all in active fetches)
      XLOG(DBG1) << "LRU track " << oldestTrackName
                 << " has active groups, cannot evict for byte limit. "
                 << "Bytes: " << totalCachedBytes_
                 << ", limit: " << maxCachedBytes_;
      break;
    }
  }
