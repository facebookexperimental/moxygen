...
void SubscriberState::Callback::onSubscribeDone(SubscribeDone done) {
  ...
  if (state_.subHandle_) {
    state_.subHandle_->unsubscribe();
...
MoQPerfTestClient::MoQPerfTestClient(
  ...
  params_.objectsPerGroup = kObjectsPerGroup;
  ...
  params_.deliveryTimeout = -1;       // don't set the server's delivery timeout
  ...
  params_.lastObjectInTrack = kObjectsPerGroup;
}
...
folly::coro::Task<void> MoQPerfTestClient::addSubscriber() {
  ...
  try {
    auto subscriber = std::make_unique<SubscriberState>(
        *this, id, evb_, url_, useQuicTransport_);
