...
int main(int argc, char** argv) {
  ...
  uint32_t maxSubscribersPerThread =
      std::max(1u, FLAGS_subscriber_ramp / FLAGS_num_threads);
  uint32_t maxSubscribersTotal = FLAGS_subscriber_max;
  uint32_t maxSubscribersPerThreadCap =
      std::max(1u, maxSubscribersTotal / FLAGS_num_threads);

  XLOG(INFO) << "Subscriber ramp (per thread): " << maxSubscribersPerThread
             << "/sec; Max subscribers (per thread): "
             << maxSubscribersPerThreadCap;
